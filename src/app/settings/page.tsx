...
export default function Settings() {
  ...
  const handleAddCharacter = () => {
    if (newName.trim() && newPrompt.trim()) {
      ...
      localStorage.setItem('custom_characters', JSON.stringify(updatedCharacters));
      setNewName('');
      setNewPrompt('');
    }
  };
                ...
                characters.map((char, index) => (
                  <div key={index} className="flex items-center justify-between p-2 border rounded-md">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{char.prompt}</p>
                    </div>
                    <Button variant="ghost" size="icon" onClick={() => handleDeleteCharacter(index)}>
                      <Trash2 className="h-4 w-4 text-destructive" />
                    </Button>
                  </div>
                ))
              ) : (
                <p className="text-muted-foreground">No custom characters added yet.</p>
              )}
            </div>
          </CardContent>
        </Card>
